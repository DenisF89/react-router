...
export const nextProduct = (id,category,categoryProducts,currentIndex,navigate) => {
        let nextId = Number(id) + 1;
        if(category){                                                               //se esiste il parametro category
            if (currentIndex < categoryProducts.length - 1)                         //se non è l'ultimo indice dell'array              
                {nextId = categoryProducts[currentIndex + 1].id;}                   //ricavo il prossimo id prodotto dal prossimo indice dell'array                
...
export const prevProduct = (id,category,categoryProducts,currentIndex,navigate) => {
        let prevId = Number(id) - 1;
        if(category){
            if (currentIndex > 0)                                                          //se non è il primo indice dell'array
                {prevId = categoryProducts[currentIndex - 1].id;}                          //ricavo l'id del prodotto con l'indice precedente
